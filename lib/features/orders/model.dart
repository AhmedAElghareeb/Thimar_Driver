class DriverOrdersData {
  late final List<DriverOrdersModel> data;
  late final Links links;
  late final Meta meta;
  late final String status;
  late final String message;

  DriverOrdersData.fromJson(Map<String, dynamic> json){
    data = List.from(json['data'] ?? []).map((e)=>DriverOrdersModel.fromJson(e)).toList();
...
class DriverOrdersModel {
  ...
  DriverOrdersModel.fromJson(Map<String, dynamic> json){
    id = json['id'] ?? 0;
    status = json['status'] ?? "";
    date = json['date'] ?? "";
    time = json['time'] ?? "";
